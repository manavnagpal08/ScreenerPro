...
# Custom stop words (should match those in streamlit_app.py)
...
# --- Helper Functions (Copied from streamlit_app.py for consistency) ---
def clean_text(text):
...
def train_screening_model():
    ...
    # 2. Generate Diverse Synthetic Data (Replace with your actual labeled dataset)
    # This dataset now covers a much wider range of roles and relevance levels.
    synthetic_data = [        # --- Additional 100 Data Points for Expanded Diversity ---
        # Example: Senior Data Analyst
        # --- Additional 100 Data Points for Expanded Diversity ---
        # Example: Senior Data Analyst# Example: Senior Product Manager (Tech)
        # --- Additional 200 Data Points for Even More Diversity and Accuracy ---
        # Example: Senior Product Manager (Tech)
        {
    ...
    print(f"Generated {len(df_train)} synthetic data points for training.")
    ...
    for index, row in df_train.iterrows():
        ...
        # Calculate keyword overlap count (filtered by STOP_WORDS)
        ...
        # Matched core skills count (using JD's top keywords)
        jd_top_skills = get_top_keywords(jd_clean, num_keywords=20)
        ...
        features = np.concatenate([
            jd_embed,
            resume_embed,
            np.array([keyword_overlap_count, resume_len, matched_core_skills_count, years_exp_for_model])
        ])
        X.append(features)
    ...
    # Using RandomForestRegressor as it's robust and often performs well
