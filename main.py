...
# --- Global Fonts & UI Styling (Re-introducing and potentially refining CSS) ---
...
if tab == "🏠 Dashboard":
    ...
    st.markdown("---") # Separator for visual hierarchy
    ...
    # Average Score and Quick Action Buttons
    ...
    with col5:
        # Using a custom div that looks like a button, and JavaScript to set session state
        st.markdown("""
        ...
        """, unsafe_allow_html=True)
        # Fallback Streamlit button if JS doesn't work or for accessibility
        if st.button("Go to Resume Screener", key="dashboard_screener_button_fallback", use_container_width=True):
    ...
    with col6:
        ...
        """, unsafe_allow_html=True)
        # Fallback Streamlit button if JS doesn't work or for accessibility
        if st.button("Go to Email Candidates", key="dashboard_email_button_fallback", use_container_width=True):
    ...
    if not df_results.empty:
        try:
            ...
            st.markdown("---") # Separator for tables/charts
            ...
            st.markdown("---") # Separator for skills
