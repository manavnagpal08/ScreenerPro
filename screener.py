...
_MASTER_SKILLS_SORTED_BY_LENGTH = sorted(list(MASTER_SKILLS), key=len, reverse=True)
...
def extract_relevant_keywords(text, filter_set):
    ...
    if filter_set: # If a specific filter_set (like MASTER_SKILLS) is provided
        # Use pre-sorted and pre-compiled patterns for MASTER_SKILLS if it's the target filter_set
        if filter_set == MASTER_SKILLS:
            sorted_filter_skills = _MASTER_SKILLS_SORTED_BY_LENGTH
            regex_patterns = _MASTER_SKILLS_REGEX_PATTERNS
        else:
            # Fallback for other filter_sets (e.g., if MASTER_SKILLS were empty and STOP_WORDS used)
        ...
        for skill_phrase in sorted_filter_skills:
            pattern = regex_patterns.get(skill_phrase.lower())
            if not pattern: # Should ideally not happen if pre-compiled correctly
                pattern = re.compile(r'\b' + re.escape(skill_phrase.lower()) + r'\b')
                regex_patterns[skill_phrase.lower()] = pattern # Cache if not pre-compiled

            matches = pattern.findall(temp_text)
            if matches:
                extracted_keywords.add(skill_phrase.lower()) # Add the original skill (lowercase)
                temp_text = pattern.sub(" ", temp_text) # Use pattern.sub for efficiency
        
        individual_words_remaining = set(re.findall(r'\b\w+\b', temp_text))
        for word in individual_words_remaining:
            if word in filter_set: # Check if the word itself is a skill
...
def generate_detailed_hr_assessment(candidate_name, score, years_exp, semantic_similarity, jd_text, resume_text, matched_keywords, missing_skills, jd_min_experience_req):
    """
    Generates a detailed, multi-paragraph HR assessment for a candidate.
    jd_min_experience_req: The minimum experience required as set by the slider.
    """
    assessment_parts = []
    overall_assessment_title = ""
    next_steps_focus = ""
    
    jd_min_exp = jd_min_experience_req

    exp_status = ""
    if years_exp < jd_min_exp:
        exp_status = f"The candidate's {years_exp:.1f} years of experience are below the minimum {jd_min_exp} years expected for this role."
    elif years_exp >= jd_min_exp + 5: # Arbitrary threshold for overqualified
        exp_status = f"With {years_exp:.1f} years of experience, the candidate appears highly experienced, potentially overqualified for a standard role at this level (JD typically requires {jd_min_exp}+ years)."
    else:
        exp_status = f"The candidate's {years_exp:.1f} years of experience align well with the {jd_min_exp} years expected for this role."

    # Tier 1: Exceptional Candidate
    if score >= 90 and years_exp >= jd_min_exp and semantic_similarity >= 0.85:
        ...
        assessment_parts.append(exp_status)
        
        if matched_keywords:
            assessment_parts.append(f"They possess a robust skill set directly matching critical keywords such as **{', '.join(matched_keywords[:5])}{'...' if len(matched_keywords) > 5 else ''}**, suggesting immediate productivity and minimal ramp-up time.")
        else:
            assessment_parts.append("Their skill set shows strong conceptual alignment with the role, indicating immediate productivity.")

        if missing_skills:
            assessment_parts.append(f"While an exceptional fit, minor areas to explore include explicit experience with: **{', '.join(missing_skills[:3])}{'...' if len(missing_skills) > 3 else ''}**. These are typically areas for quick adaptation.")
        else:
            assessment_parts.append("No significant skill gaps were identified based on the provided job description, indicating a comprehensive match.")
    ...
    # Tier 2: Strong Candidate
    elif score >= 80 and years_exp >= jd_min_exp * 0.75 and semantic_similarity >= 0.7: # A bit more lenient on exp for "strong"
        ...
        if matched_keywords:
            assessment_parts.append(f"Key strengths include a significant overlap in required skills, notably **{', '.join(matched_keywords[:5])}{'...' if len(matched_keywords) > 5 else ''}**, and practical experience that directly addresses the job's demands.")
        else:
            assessment_parts.append("Their experience shows strong conceptual alignment with the role, indicating effective contribution.")

        if missing_skills:
            assessment_parts.append(f"Minor areas for discussion could include: **{', '.join(missing_skills[:3])}{'...' if len(missing_skills) > 3 else ''}**. These appear to be areas where the candidate could either quickly adapt or has indirect experience.")
        else:
            assessment_parts.append("Few to no significant skill gaps were identified, indicating a well-rounded profile for this role.")
        
        next_steps_focus = "During the interview, explore specific project methodologies, problem-solving approaches, and long-term career aspirations to confirm alignment with team dynamics and growth opportunities within the company. Focus on how they would address any minor skill gaps."
    # Tier 3: Promising Candidate
    elif score >= 60 and years_exp >= jd_min_exp * 0.5 and semantic_similarity >= 0.35: # More lenient on exp for "promising"
        ...
        gaps_identified = []
        if years_exp < jd_min_exp * 0.75:
            gaps_identified.append(f"Experience ({years_exp:.1f} yrs) is somewhat below the {jd_min_exp} years expected; assess their ability to scale up quickly and take on more responsibility.")
        
        if missing_skills:
            gaps_identified.append(f"Key skills from the job description not explicitly highlighted in the resume include: **{', '.join(missing_skills[:5])}{'...' if len(missing_skills) > 5 else ''}**. It's crucial to probe these areas.")
        else:
            gaps_identified.append("While no explicit missing skills, the semantic similarity suggests a need to confirm practical application of skills.")

        if matched_keywords:
            assessment_parts.append(f"The resume does show some relevant keywords like **{', '.join(matched_keywords[:3])}{'...' if len(matched_keywords) > 3 else ''}**, indicating a foundational understanding.")
        
        if gaps_identified:
            assessment_parts.append("Areas for further exploration include: " + " ".join(gaps_identified))
        
        next_steps_focus = "The interview should focus on validating foundational skills, understanding their learning agility, and assessing their potential for growth within the role. Be prepared to discuss specific examples of how they've applied relevant skills and how they handle challenges. Explore their motivation for this specific role."
    # Tier 4: Limited Match
        ...
        main_concerns = []
        if years_exp < jd_min_exp * 0.5:
            main_concerns.append(f"Experience ({years_exp:.1f} yrs) is significantly below the expected {jd_min_exp} years.")
        
        if missing_skills:
            main_concerns.append(f"A large number of critical skills from the job description are missing or not clearly demonstrated, such as: **{', '.join(missing_skills[:5])}{'...' if len(missing_skills) > 5 else ''}**.")
        elif semantic_similarity < 0.2:
            main_concerns.append("The overall semantic similarity is very low, suggesting a fundamental mismatch in domain or core competencies.")

        if main_concerns:
            assessment_parts.append("Key concerns include: " + " ".join(main_concerns))
        else:
            assessment_parts.append("The profile lacks sufficient alignment with the job description's core requirements.")

        assessment_parts.append("While some transferable skills may exist, a substantial investment in training or a re-evaluation of role fit would likely be required for this candidate to succeed. This candidate is generally not recommended for the current role unless there are specific, unforeseen niche requirements or a strategic need to broaden the candidate pool significantly.")
        next_steps_focus = "If proceeding, focus on understanding their fundamental capabilities and long-term career aspirations, and be transparent about the significant skill gap. Otherwise, consider for future roles that align better with their existing profile."
...
def resume_screener_page():
    ...
    with col1:
        ...
        if jd_option == "Upload my own":
            ...
            if jd_file:
                jd_text = jd.file.read().decode("utf-8")
        else:
    ...
    if jd_text and resume_files:
        ...
        for i, file in enumerate(resume_files):
            ...
            detailed_hr_assessment = generate_detailed_hr_assessment(
                ...
                jd_min_experience_req=min_experience
            )
...
def analytics_dashboard_page():
    ...
    def load_screening_data():
        """
        Loads screening results from session state. If no data is in session state,
        it explicitly states that the screener needs to be run first.
        """
